...
bool AArch64LoadStoreOpt::tryToPairLdStInst(MachineBasicBlock::iterator &MBBI) {
  ...
  if (Paired != E) {
    ...
    if (Subtarget->hasLdpAlignedOnly() || Subtarget->hasStpAlignedOnly()) {
      ...
      // If a store arrives and stp-aligned-only feature is opted, check that
      // the alignment of the source pointer is at least double the alignment of
      // the type.
      if (MI.mayStore() && Subtarget->hasStpAlignedOnly() &&
          MemAlignment < 2 * TypeAlignment) {
        NumFailedAlignmentCheck++;
        return false;
      }
